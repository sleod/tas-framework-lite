...
import ch.sleod.testautomation.framework.common.utils.ZipUtils;
...
public class ExternAppController {
    ...
    public static Path matchChromeAndDriverVersion() throws IOException {
        ...
        if (paths.isEmpty() || driverPath == null) {
            warn("No File matched to current chrome browser! Try to download from TFS source!");
            String folder = downloadChromeDrivers();
            paths = FileLocator.listRegularFilesRecursiveMatchedToName(folder, 3, "chromedriver");
            driverPath = checkPaths(paths, chromeVersion);
        }
        if (driverPath != null) {
            trace("Find Driver: " + driverPath.toFile().getName() + " with Version: " + chromeVersion);
            return driverPath;
        } else {
    ...
    private static Path checkPaths(List<Path> paths, String chromeVersion) {
        ...
        for (Path path : paths) {
            ...
            if (matcher.find()) {
                ...
                warn("No Version was found with given Pattern like 'xx.xxx.xxx.xxx' -> " + response);
                warn("The Chrome Driver may too old! Please Delete it.");
            }
        }
        return driverPath;
    }
    ...
    public static String getCurrentChromeVersion() {
        ...
        if (matcher.find()) {
            ...
            warn("No Version was found with given Pattern -> " + response);
            return response;
        }
    }
